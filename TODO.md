...
- [x] Move logging and other utility functions from integration_tests.py and vm_controller.py into
  ...
  coupled functions.
  - Created utils.py module with common functionality
  - Extracted ColorFormatter, setup_logging, cleanup_old_logs, get_last_lines
  - Extracted ProcessWithOutput and run_subprocess functions
  - Updated both vm_controller.py and integration_test.py to use utils module
  - Fixed issues with subprocess parameter handling (capture_output, check, timeout)

- [x] Need to reduce code duplication in agent-vm. Scan for the FIXME and TODO I
  left you.
...
## 🚀 Future: Haskell Migration

- [ ] **Haskell Migration Analysis** ✅
  - [x] Research best Haskell libraries for VM management
  - [x] Create migration plan in HASKELL_MIGRATION.md
  - [x] Document benefits of type safety for VM state management
  - [x] Define comprehensive integration test suite (50 tests)
  - [x] Specify contravariant logging approach with co-log
  - [x] Plan library-first architecture for daemon integration
  - [ ] Prototype core VM controller in Haskell
  - [ ] Implement type-safe state machines for VM lifecycle
  - [ ] Create STM-based concurrent VM registry
  - [ ] Set up haskell.nix for static linking with musl
  - [ ] Port all 50 integration tests to hspec

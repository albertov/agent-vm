...
interactWith :: (MonadIO m, InteractiveBackend backend) => Handle -> Handle -> Handle -> backend -> m ()
interactWith hStdin hStdout hStderr backend = do
  ...
  liftIO $
    bracket
      ...
      ( \_ -> withInteractive 1000 backend $ \socket -> do
          hPutStrLn hStdout "\r\nConnected to VM serial console. Press Ctrl+A then 'x' to exit."
          hPutStrLn hStdout "Note: Use Ctrl+A x to exit cleanly. Ctrl+D will exit the shell but not the console."
          hFlush hStdout
          -- Use race_ to ensure both threads exit when one finishes/fails
          race_
            (inputRelayWithEscape hStdin socket)
            (outputRelay hStdout socket)
      )
  where
    -- Input relay with escape sequence detection (Ctrl+A x to exit)
    inputRelayWithEscape hIn socket = do
      let loop escapeMode = do
            ...
            if BS.null input
              then return () -- EOF on stdin, exit gracefully
                ...
                -- Check for escape sequence: Ctrl+A (0x01) followed by 'x'
                shouldExit <- handleEscapeSequence escapeMode input socket hStderr
                unless shouldExit $ loop False
      loop False

    -- Handle escape sequence detection and normal input
    handleEscapeSequence escapeMode input socket hErr = do
      ...
      case (escapeMode, inputList) of
        ...
        (True, 120 : _) -> do
          ...
          return True
        (True, _) -> do
          -- Not 'x', send the Ctrl+A and current input
          writeBytes socket (BS.pack [1]) -- Send Ctrl+A
          writeBytes socket input
          return False
        -- Check for Ctrl+A (0x01)
        (False, [1]) -> return False -- Enter escape mode, don't send Ctrl+A yet
        (False, 1 : rest) -> do
          -- Ctrl+A followed by other chars
          case rest of
            120 : _ -> do
              -- 'x'
              hPutStrLn hErr "\r\nExiting..."
